...
class TelegramLinkSerializer(serializers.Serializer):
    """Telegram akkauntni ulash uchun."""
    telegram_id = serializers.CharField(max_length=20)

    def validate_telegram_id(self, value):
        from .models import CustomUser
        if CustomUser.objects.filter(telegram_id=value).exclude(
            pk=self.context['request'].user.pk
        ).exists():
            raise serializers.ValidationError("Bu Telegram ID boshqa akkauntga bog'langan.")
        return value
