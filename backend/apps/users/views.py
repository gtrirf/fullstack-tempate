...
from .permissions import IsRoleAllowed, IsBotService
...
# ---------------------------------------------------------------------------
# Bot service endpoint'lari
# ---------------------------------------------------------------------------

class UserByTelegramView(APIView):
    """GET /users/by-telegram/<telegram_id>/ — bot uchun Telegram ID bo'yicha user."""
    permission_classes = [IsBotService]

    @extend_schema(
        tags=['Bot'],
        summary='Telegram ID orqali user olish',
        description='Faqat bot servis uchun (X-Bot-Secret header talab qilinadi).',
        responses={200: UserDetailSerializer, 404: _error_response},
    )
...
class LinkTelegramView(APIView):
    """POST /users/link-telegram/ — Telegram akkauntni ulash."""
    permission_classes = [IsAuthenticated]

    @extend_schema(
        tags=['Profile'],
        summary='Telegram akkauntni ulash',
        request=TelegramLinkSerializer,
        responses={200: _msg_response, 400: _error_response},
    )
    def post(self, request):
        serializer = TelegramLinkSerializer(data=request.data, context={'request': request})
        serializer.is_valid(raise_exception=True)
        request.user.telegram_id = serializer.validated_data['telegram_id']
        request.user.save(update_fields=['telegram_id'])
        logger.info("Telegram linked for user %s: %s", request.user.email, request.user.telegram_id)
        return Response({'message': "Telegram akkaunt muvaffaqiyatli ulandi."})


class UnlinkTelegramView(APIView):
    """DELETE /users/unlink-telegram/ — Telegram akkauntni uzish."""
    permission_classes = [IsAuthenticated]

    @extend_schema(
        tags=['Profile'],
        summary='Telegram akkauntni uzish',
        responses={200: _msg_response},
    )
    def delete(self, request):
        request.user.telegram_id = None
        request.user.save(update_fields=['telegram_id'])
        logger.info("Telegram unlinked for user %s", request.user.email)
