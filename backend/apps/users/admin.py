...
@admin.action(description='Telegram orqali xabar yuborish')
def send_telegram_notification_action(modeladmin, request, queryset):
    from .notifier import notify_user
    sent = 0
    skipped = 0
    for user in queryset:
        if user.telegram_id:
            notify_user(user.telegram_id, "<b>Admin xabari</b>\n\nSizga admin tomonidan xabar yuborildi.")
            sent += 1
        else:
            skipped += 1
    msg = f"{sent} ta foydalanuvchiga Telegram xabari yuborildi."
    if skipped:
        msg += f" {skipped} ta foydalanuvchida Telegram bog'lanmagan."
    modeladmin.message_user(request, msg, messages.SUCCESS if sent else messages.WARNING)
...
class CustomUserAdmin(admin.ModelAdmin):
    ...
    actions = [send_bulk_email_action, send_telegram_notification_action]
